  ...
  "inline-flex items-center justify-center whitespace-nowrap cursor-pointer rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90 border border-destructive-foreground/10 hover:no-underline",
        success:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      ...
      >
        {isLoading ? <Loader className="w-16 h-4 mx-2" /> : props.children}
      </Comp>
    );
  }
);
